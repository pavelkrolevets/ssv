...
func NewSimulatedBackendWithDatabase(database ethdb.Database, alloc core.GenesisAlloc, gasLimit uint64) *SimulatedBackend {
	...
	ethservice, err := eth.NewWithBlockchain(n, config, blockchain)
	if err != nil {
		panic(err)
	}
	...
	backend.filterSystem = filters.NewFilterSystem(ethservice.APIBackend, filters.Config{})
	backend.events = filters.NewEventSystem(backend.filterSystem, false)

	header := backend.blockchain.CurrentBlock()
	block := backend.blockchain.GetBlock(header.Hash(), header.Number.Uint64())

	backend.Node.RegisterAPIs([]rpc.API{{
		Namespace: "eth",
		Service:   filters.NewFilterAPI(backend.filterSystem, false),
	}})

	if err := backend.Node.Start(); err != nil {
		panic(err)
	}
